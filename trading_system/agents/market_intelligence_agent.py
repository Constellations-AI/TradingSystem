...
class MarketIntelligenceAgent:
    ...
    async def setup(self):
        ...
        intel_analyst_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.3)
        eval_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.3)
        self.evaluator_llm_with_output, self.evaluator = build_market_intel_evaluator(eval_llm)
        # Note: build_graph will be called in run_superstep with session-specific tools

    def intel_analyst(self, state: State) -> Dict[str, Any]:
        state = self._ensure_success_criteria(state)
        system_prompt = f""" You are a market intelligence analyst who can use tools to analyze the market. You keep analyzing the topic given to you by the user until you have a question or clarification for the user, or the success criteria is met.
        You have many tools at your disposal to help you analyze the market. Please use them to formulate your response and do not make up any information. 
        ...
        response = self.intel_analyst_llm_with_tools.invoke(messages)
        response.metadata = {"sender": "intel_analyst"}
    ...
    def intel_analyst_router(self, state: State) -> str:
    ...
    async def build_graph(self):
        ...
        graph_builder.add_node("intel_analyst", self.intel_analyst)
        graph_builder.add_node("tools", ToolNode(self.tools))
        graph_builder.add_node("evaluator", self.evaluator.evaluate)
        graph_builder.add_conditional_edges("intel_analyst", self.intel_analyst_router, {"tools": "tools", "evaluator": "evaluator"})
        graph_builder.add_edge("tools", "intel_analyst")
        graph_builder.add_conditional_edges("evaluator", self.route_based_on_evaluation, {"intel_analyst": "intel_analyst", "END": END})
        graph_builder.add_edge(START, "intel_analyst")
    ...
    async def run_superstep(self, message, success_criteria, history, debug=False):
        ...
        self.tools = await market_intelligence_tools(session_id)
        intel_analyst_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.3)
        ...
        analyst_reply = get_last_message_by_sender(result["messages"], "intel_analyst")
